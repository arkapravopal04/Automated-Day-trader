#!/usr/bin/env bash
# STEP 6 (AMENDED) -- THE RE-FROZEN REFERENCE RUN, under Appendix A.
#
...
# logs/p3/on/freeze2/ (the reference frozen under the RELATIVE cap, retained as
# the record of the defect) and logs/p3/on/cap2/ (the amended arms) all survive.
#
# The variant is the `both2` arm: min(KAPPA/n_selected, A) with KAPPA = 3.0 and
# A = 0.10, plus the earnings exclusion. Appendix A of the pre-registration
# declared it the reference before any of the four amended arms had been run.
set -u
cd "$(dirname "$0")"
mkdir -p logs/p3/on/freeze3
...
for k in 1 2 3 4 5; do
  ...
  env TRADING_TRAIN_FRAC=$TF TRADING_VAL_FRAC=0.10 \
    ...
    python -u eval/xsec_book.py --overnight --edge reversal --risk-scale vol \
      ...
      --min-names-frac 0.20 \
      --max-weight-mult $KAPPA --max-weight-frac $ABS --earnings-calendar $CAL \
      --json logs/p3/on/freeze3/freeze3_f${k}.json \
      > logs/p3/on/freeze3/freeze3_f${k}.log 2>&1
  echo "    exit $?"
done
echo "=== RE-FREEZE (APPENDIX A) WALK-FORWARD COMPLETE ==="
