...
class Tensor:
    ...
    def sigmoid(self):
        clipped_pos = np.clip(-self.data, -500, 500)
        clipped_neg = np.clip(self.data, -500, 500)
        
        s = np.where(
            self.data >= 0,
            1.0 / (1.0 + np.exp(clipped_pos)), 
            np.exp(clipped_neg) / (1.0 + np.exp(clipped_neg))  
        )
        ...
        def _backward():
            ...
            if axis is None:
                ...
                if not keepdims:
                    ...
                    axes = [a if a >= 0 else len(shape) + a for a in axes]
                    for a in axes:
                        shape[a] = 1
                    grad_to_broadcast = grad.reshape(shape)
                else:
                    grad_to_broadcast = grad
                
                num_elements = self.data.size / out.data.size
                self.grad += np.ones_like(self.data) * grad_to_broadcast / num_elements
    ...
    def softmax(self, axis=-1):
        ...
        shifted = Tensor(self.data - max_val, (self,), 'softmax_shift')

        def _shift_backward():
            self.grad += shifted.grad

        shifted._backward = _shift_backward

        clamped_data = np.clip(shifted.data, -20.0, 0.0)
        e = np.exp(clamped_data)
        s = e / (e.sum(axis=axis, keepdims=True) + 1e-8)

        out = Tensor(s, (shifted,), 'softmax')

        def _backward():
            dot = (out.grad * s).sum(axis=axis, keepdims=True)
            shifted.grad += s * (out.grad - dot)
